...
async function openReel(id) {
  ...
  ${r.avg_watch_time ? `<div class="block" style="display:flex;justify-content:space-between;gap:10px;align-items:center"><span><b>Retención</b> · la gente ve en promedio <b>${r.avg_watch_time.toFixed(1)} s</b>${r.duration ? ` de ${Math.round(r.duration)} s` : ''}</span>${r.retention != null ? `<span class="pill ${r.retention >= 40 ? 'good' : r.retention >= 20 ? '' : 'bad'}">${r.retention}%</span>` : ''}</div>` : ''}
