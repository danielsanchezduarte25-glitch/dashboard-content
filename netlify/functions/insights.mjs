...
${top5?.patterns?.length ? `\nPATRONES HISTÓRICOS YA DETECTADOS (top 5 de todos los tiempos): ${top5.patterns.join(' | ')}` : ''}`;
  ...
  // Two lighter passes (so no answer gets cut by max_tokens): 1) qué/por qué/ángulos, 2) hooks/frases/recomendaciones.
  const ask = async (prompt, okKey, max_tokens) => {
    let lastErr = null;
    for (let attempt = 0; attempt < 2; attempt++) {
      try { const text = await claude({ system: sys, messages: [{ role: 'user', content: prompt }], max_tokens, temperature: 0.45 }); const j = extractJSON(text); if (j?.[okKey]) return j; lastErr = new Error('respuesta sin JSON válido: ' + text.slice(0, 120)); }
      catch (e) { lastErr = e; }
    }
    throw Object.assign(new Error('La IA no devolvió los insights (' + (lastErr?.message || '') + '); intentá de nuevo.'), { status: 502 });
  };
  const a1 = await ask(`${facts}\n\nPRIMERA PARTE. Devolvé SOLO este JSON:\n{"headline":"una frase con la lectura principal del período","worked":[{"title":"título del reel","metric":"el dato que lo prueba (ej. 3.2× mediana, 41% retención)","reason":"por qué funcionó en 1-2 frases"}],"why":"explicación de 4-6 líneas del patrón de fondo: qué tienen en común los que funcionaron y qué les falta a los que no (emoción, utilidad, identidad, curiosidad, prueba social, formato)","angles":[{"angle":"nombre del ángulo ganador","evidence":"qué reels lo prueban y con qué números","howToUse":"cómo explotarlo en los próximos videos"}],"stop":["cosa concreta que conviene dejar de hacer 1","2"]}\n\nReglas: 3-5 elementos en worked, 3-4 en angles. Frases cortas, sin comillas dobles dentro de los textos.`, 'worked', 3000);
  const a2 = await ask(`${facts}\n\nSEGUNDA PARTE (lectura previa: ${a1.headline || ''}). Devolvé SOLO este JSON:\n{"hooks":[{"hook":"hook literal (primeros 3 s) o reconstruido si no hay transcripción","reel":"título del reel","why":"por qué engancha","formula":"fórmula reutilizable con huecos entre corchetes"}],"phrases":[{"phrase":"frase literal que sostiene la atención o dispara guardados/compartidos","reel":"título del reel","why":"qué hace esa frase (recontextualiza, promete, crea tensión, da el dato…)"}],"recommendations":[{"title":"recomendación corta","detail":"qué hacer exactamente y por qué, con el dato que lo respalda"}]}\n\nReglas: 4-6 hooks, 4-6 frases, 4-6 recomendaciones. Si no hay transcripciones, decilo y trabajá con captions y métricas. Sin comillas dobles dentro de los textos.`, 'hooks', 3500);
  const analysis = { ...a1, ...a2 };
