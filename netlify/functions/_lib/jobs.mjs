...
import { getJSON, setJSON, setWorkspace, getWorkspace, K } from './store.mjs';
...
export async function runAsJob(req, kind, params) {
  ...
  const id = randomUUID(); const ws = getWorkspace();
  await setJSON(K.job(id), { id, kind, ws, status: 'queued', created: new Date().toISOString() });
  try {
    await fetch(`${siteUrl(req)}/api/jobs-background?id=${id}`, { method: 'POST', headers: { 'x-sync-secret': internalSecret(), 'content-type': 'application/json' }, body: JSON.stringify({ kind, params, ws }) });
  } catch (e) {
...
export async function executeJob(id, kind, params, ws = 'main') {
  setWorkspace(ws);
  await setJSON(K.job(id), { id, kind, ws, status: 'running', started: new Date().toISOString() });
  try {
