...
globalThis.fetch = async (url, init) => {
  ...
  if (u.startsWith('https://api.anthropic.com')) { const body = JSON.parse(init.body); const last = body.messages.at(-1).content; if (/"items":\[\{"index"/.test(last)) return j({ content: [{ type: 'text', text: JSON.stringify({ summary: 'Los 5 comparten hooks directos.', patterns: ['Hook en 2 s', 'CTA con palabra clave'], items: [1, 2, 3, 4, 5].map((i) => ({ index: i, title: 'V' + i, hook: 'h', script: 's', structure: 'e', behavior: 'b', retention: 'r', why: 'w', replicate: 'rep', template: '[tema]' })) }) }] }); const isJSON = /Devolvé SOLO un JSON/.test(last); return j({ content: [{ type: 'text', text: isJSON ? '{"hook":"Buen hook","retention":"Ok","cta":"Comentá IA","improve":"Probá X","score":72,"keyword":"IA","title":"Reel test","why":"w","onscreen":"o","body":"b","publish":"p"}' : '## Idea\n- Punto uno\n> "Hook"' }] }); }
